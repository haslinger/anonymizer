class DownloadsController < ApplicationController
  ...
  def report
    @downloads = Episode.all
                        ...
                        .select("episodes.name,
                                 episodes.podcast,
                                 downloads.*,
                                 count(downloads.id) as download_count")


    # get integer from episode /w*(\d+)/.match("ieiaeuiae01")[1].to_i
  end
  ...
  private
    ...
    def download_params
      params.require(:download).permit(:episode_id, :day, :size)
    end
end
